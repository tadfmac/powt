...
class powtServer {
  ...
  start(config){
    ...
    (async ()=>{
      await this._startSessionReader(this.path);
    })();
  }
  getSessionIds(){
    return Object.keys(this.sessions);
  }
  async _startSessionReader(path){
    Log.out("powtServer._startSessionReader()");
    try{
      ...
      while(!this.isKilled){
        ...
        session.value.ready.then(async () => {
          ...
          if(session._userData == undefined){
            ...
            this._bidiReadableWaiting(reader,session._userData.id).catch((e)=>{
            ...
            this._dgramReadableWaiting(dgramReader,session._userData.id).catch((e)=>{
      ...
      Log.out("powtServer._startSessionReader failed : "+e);
    };
  }
  async send(id,uint8Arr){
    ...
    if(this.sessions[id]){
      if(this.sessions[id]._userData.bidiWriter){
        ...
        this._addUint8Arr(buffer,8,uint8Arr);
  ...
  async _bidiReadableWaiting(reader,id){
    while(true){
      ...
      if(this.sessions[id]._userData._bidiData.progress == false){
        ...
        if(totalLength > value.length){
          ...
          this._addUint8Arr(this.sessions[id]._userData._bidiData.buffer,0,value);
        }else{
        ...
        this._addUint8Arr(this.sessions[id]._userData._bidiData.buffer,this.sessions[id]._userData._bidiData.rSize,value);
        this.sessions[id]._userData._bidiData.rSize += value.length;
  ...
  async _dgramReadableWaiting(reader,id){
    while(true){
      const { done, value } = await reader.read();
      if (done) {
        break;
  ...
  _addUint8Arr(buff,index,arr){
    Log.out("powtServer._addUint8Arr buffSize="+buff.length+" index="+index+" arrSize="+arr.length);
  ...
  on(event,_func){
    ...
    case "open":
    case "new":
      this.onNewSession = _func;
      break;
    case "closedsession":
    case "closed":
    case "closesession":
    case "close":
      this.onCloseSession = _func;
      break;
    case "stream":

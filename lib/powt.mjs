...
class powt {
    ...
    while (true) {
      ...
      if(this._bidiData.progress == false){
        ...
        if(totalLength > value.length){
          ...
          this.addUint8Arr(this._bidiData.buffer,0,value);
        }else{
        ...
        this.addUint8Arr(this._bidiData.buffer,this._bidiData.rSize,value);
        this._bidiData.rSize += value.length;
  ...
  async send(uint8Arry){
    ...
    if(this._bidiWriter != null){
      ...
      this.addUint8Arr(buffer,8,uint8Arry);
  ...
  addUint8Arr(buff,index,arr){
    console.log("powt.addUint8Arr buffSize="+buff.length+" index="+index+" arrSize="+arr.length);
